...
//     stopProgramX();
//     std::cerr << "Erreur lors de l'initialisation de la souris." << std::endl;
//   }
 
//   if(has_mouse() != TRUE) {
//     stopProgramX();
//     std::cerr << "Aucune souris n'est détectée." << std::endl;
//   }
// }
...
void Window::print(WINDOW* Win, int x, int y, char s, Color c) const{
  wattron(Win,COLOR_PAIR(c));
  mvwaddch(Win,y,x,s);
  wattroff(Win,COLOR_PAIR(c));
  update();
...
WINDOW* Window::getWindowWin() const { return win;}
